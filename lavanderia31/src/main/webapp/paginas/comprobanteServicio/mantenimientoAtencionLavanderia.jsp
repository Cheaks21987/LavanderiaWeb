				...
				<tr style="height:55px;">
					<td style="width:7%;background-color:#191970;font-weight: bold;font-size:20px;color:white;">
		              ...
		              // lo que el usuario escribi�
		                ...
		                if (e.charCode === 13) {
		                $.ajax({
		                    url: "<%=request.getContextPath()%>/ServletMantenimientoPersona?action=buscarClienteLavanderia",
															data : {
																filtro : txtFiltro
															},
				        ...
				        btn2.onclick = (event) => {
				        var txtFiltro2 = $("#idCliente").val();
				        $.ajax({
		                    url: "<%=request.getContextPath()%>/ServletMantenimientoPersona?action=crearBuscar",
															data : {
																filtro : txtFiltro2
						...
						function generarC(){
							const btn2 = document.querySelector('#btnG');
					        btn2.onclick = (event) => {
					        var txtFiltro2 = $("#idCliente2").val();
					        $.ajax({
			                    url: "<%=request.getContextPath()%>/ServletMantenimientoPersona?action=nuevoC",
																data : {
																	filtro : txtFiltro2
